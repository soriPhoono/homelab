{
  ...
  cfg = config.hosting.single-node;
in
  with lib; {
    imports = [
      ./modules
    ...
    options.hosting.single-node = {
      enable = mkEnableOption "Enable single-node docker container hosting";
      ...
      systemd.services =
        {
          ...
          "docker-install-plugins" = {
            ...
            serviceConfig = {
              Type = "oneshot";
              ExecStart = "${pkgs.writeShellApplication {
                name = "docker-install-plugins.sh";
                runtimeInputs = with pkgs; [
                  docker
                ];
