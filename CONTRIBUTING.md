...
- **Configuration Overlays**: When you enable a module (e.g., `services.my-service.enable = true;`), it overlays its specific settings (packages, systemd services, environment variables, etc.) onto the existing system configuration tree, for a module to be complete and accurate it must handle all forms of both it's configuration and it's collisions with other module configurations.
