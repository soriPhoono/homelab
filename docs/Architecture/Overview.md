...
2.  **Filter**: It looks for directories containing a `default.nix` or standalone `.nix` files.
3.  **Meta Injection**: It reads `meta.json` from the directory to determine the architecture (`system`) of the target.
4.  **MapAttrs**: It maps these discovered paths to `nixosSystem` or `homeManagerConfiguration` builders.
