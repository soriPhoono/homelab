...
in {
  options.core.networking.mullvad = with lib; {
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    {
      ...
      networking.firewall.checkReversePath = lib.mkIf cfg.relaxReversePathFilter (lib.mkDefault false);
    ...
    (lib.mkIf (cfg.allowLocalNetwork || cfg.disableLockdownMode || cfg.setDefaultDns) {
      systemd.services.mullvad-cli-bootstrap = {
        ...
        script = ''
          ...
          for _ in $(seq 1 30); do
            if ${lib.optionalString cfg.setDefaultDns ''
            "$mullvad" dns set default &&
          ''}${lib.optionalString cfg.disableLockdownMode ''
            "$mullvad" lockdown-mode set off &&
          ''}${lib.optionalString cfg.allowLocalNetwork ''
